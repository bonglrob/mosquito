...
def main() -> None:
    ...
    species3_count = species3.groupby(['year', 'species']).agg({'individualCount': 'sum'}).reset_index()
    print(species1_count.columns)


    print(species1_count)
    all_mosquitoes = pd.concat([species1_count, species2_count, species3_count]).reset_index(drop=True)
    print(all_mosquitoes)
